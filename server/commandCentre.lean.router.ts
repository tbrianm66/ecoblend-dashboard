...
function deriveVentureMetrics(b: VentureBundle) {
  ...
  // Stage-gate readiness for current stage.
  ...
  const stageGateReady = stageGateChecklist.length > 0 && stageGateChecklist.every((i) => i.complete) && !!nextStage(stage);
  ...
  decisionBoard: publicProcedure.query(async () => {
    ...
    return bundles.map((b) => {
      ...
      return {
        ...
        currentHypothesis: topHyp?.hypothesisStatement ?? null,
        evidenceConfidence: m.evidenceConfidence, riskScore: m.riskScore, wtpScore: m.wtpScore,
